let roomsImagesArr = [
    ...
    {key: '102', src: "https://i.ibb.co/RYjGpN9/Belle-102.png"  },
...
let apartmentListWrapperRight = document.getElementById('apartments_list_right')
...
function CreateApartmentsMarkup(){
    ...
    apartmentListDataRight.map((apartment)=>{
        let rooms = ''
        apartment.rooms.forEach(room=>rooms += `<span id=${room}>${room}</span>`);
...
function AddListenersOnApartmentsListItems(){
    ...
    let apartmentsListRight = document.getElementById('apartments_list_right').children;
